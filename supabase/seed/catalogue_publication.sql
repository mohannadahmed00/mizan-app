-- Mizan — spec 007. Publishes the catalogue version the app already seeds locally,
-- so a device that pulls sees exactly the version it would have derived offline.
--
-- Insert-only, and a published version is immutable: rewriting one would re-score
-- every day recorded against it (Principle III, contracts/remote-schema.sql).
-- ON CONFLICT DO NOTHING makes re-running this file harmless.
--
-- Payload is byte-identical to domain/src/main/resources/catalogue/valid-catalogue.json.

insert into public.catalogue_publications (version, effective_from, format_version, payload)
values (1, date '2026-01-01', 1, $catalogue${
  "versions": [
  ...
  "tasks": [
    ...
    { "slug": "adhkar", "sectionId": "adhkar", "displayPosition": 1, "label": "Adhkar" },
  ...
  "taskVersions": [
    ...
    { "taskSlug": "adhkar", "catalogueVersion": 1, "points": 2, "maxOccurrencesPerDay": 9, "scheduleRule": { "type": "everyDay" } },
