...
migration, 2 new local tables, 5 new remote tables, 1 scheduled aggregation job, 3 new domain
interfaces, 4 new pure domain functions, 48 functional requirements, 17 success criteria.

**Five decisions settled by `/speckit-clarify` after this plan was first written**, each folded back
through the design rather than left in the spec alone:

1. A participant who opts in mid-period is scored over the **whole** period (FR-021a). The
   aggregation does not filter completions by consent date, and FR-002b requires the opt-in copy to
   say so.
2. The Honor Board exists for **weekly and monthly only** (FR-027a). A days-engaged threshold over
   one day can only be 0 or 1; `honor_board_closed` is constrained to those two kinds.
3. Ties are broken by **who reached the total earliest** (FR-022), using the device-reported
   `recorded_at`. FR-022a bounds the resulting exposure explicitly — see the Complexity Tracking
   note below.
4. **No settlement window.** A period freezes at its boundary (FR-025). This removed the `SETTLING`
   state entirely: the lifecycle is `OPEN → CLOSED`.
5. Display names **need not be unique** (FR-007a). The viewer's own row is marked, so
   self-identification never depends on the name.
...
| **IV — Offline-first** | Yes, with one recorded tension | The leaderboard is a read-only remote surface with a local cache. Record, undo, view and score gain no network call and no new suspend point. The UI observes Room; the network writes into Room and never reaches a ViewModel. FR-034 and SC-008 require the whole Phase 2–7 product to behave identically with the leaderboard unreachable. **The tension**: FR-025 freezes a period at its boundary with no settlement window, so a participant who records offline and syncs afterwards scores nothing for those days *on the leaderboard*. Offline recording is not blocked, delayed or lost — FR-025a keeps the record itself whole, and SC-016 asserts both halves — but on this one surface an offline user is measurably worse off than an online one, which is the thing Principle IV exists to prevent. It is an accepted product tradeoff, taken deliberately, disclosed to the participant before they opt in (FR-025b), recorded in the spec's Assumptions with a revisit trigger, and logged in Complexity Tracking below rather than presented as clean compliance. |
...
| **IV** | Yes, tension recorded | The read model is cache-backed; [contracts/repositories.md](./contracts/repositories.md) has no method that blocks a record or view path, and `RankingState` has no `Empty` a view could render as "nobody is ahead of you". The no-settlement-window tradeoff is logged in Complexity Tracking, disclosed by FR-025b and asserted by SC-016 — not presented as clean compliance. |
...
| An offline participant scores nothing for days synced after a period freezes (Principle IV tension) | The user chose an immediate freeze over a settlement window (clarification session 2026-08-29), so that a period is final and unambiguous the moment it ends rather than provisional for a further window. | **A 48-hour settlement window** was proposed and rejected: it would have covered ordinary offline stretches and reconciled Principle III with Principle IV, at the cost of a week's standings staying unsettled into the next week. The tradeoff is bounded by FR-025a (the person's own record is untouched), disclosed by FR-025b, and asserted by SC-016. Revisit if offline participants prove observably disadvantaged. |
| The tie-break trusts a device-reported timestamp (FR-022) | Ties are broken by who reached the total earliest, which is the only rule reflecting something the participant actually did. The only available "when" is the completion's `recorded_at`, set on the device. | **Server arrival time**: unforgeable, and rejected because it would systematically place offline participants below online ones in every tie — the same Principle IV problem as above, applied to ordering. **An arbitrary stable discriminator**: unforgeable and meaningless; an observant user who is always below the same person has no explanation. FR-022a bounds the exposure: a forged clock cannot change any total, days-engaged figure or region, and cannot lift anyone above a higher total. It reorders ties and nothing else. |
