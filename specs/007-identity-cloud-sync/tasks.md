...
- [ ] T129 [US5] Run all four suites plus quickstart §4 SC-007 by hand: fresh install, airplane mode, no account, walk the whole Phase 2–6 product. **Automated part done** (`:domain:test`, `:app:test`, `:data:connectedAndroidTest` 111/111 +1 skip, `:app:connectedAndroidTest` 81/81, all green). The fresh-install/airplane-mode manual walkthrough is a human task, same as T072/T089/T109/T115 — left unchecked.
...
- [ ] T130 Re-run the row-level-security verification as the merge gate (SC-008): `supabase db execute --file specs\007-identity-cloud-sync\contracts\rls-verification.sql`. Must print `RLS OK`. Confirm `supabase/migrations/0001_identity_cloud_sync.sql` is still byte-identical to `specs/007-identity-cloud-sync/contracts/remote-schema.sql`. **Blocked in this environment**: no `supabase login` session or DB credentials are available here (only the anon key in `local.properties`, which RLS itself is what's being verified against). Needs a human with the project's Supabase CLI access, same as T007/T008 originally.
...
- [ ] T136 Run the full quickstart §4 validation end to end, all eleven success criteria, and record the result in the pull request description. Manual, same as T072/T089/T109/T115/T129 — left unchecked.
