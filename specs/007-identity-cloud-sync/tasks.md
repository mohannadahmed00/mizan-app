...
- [ ] T115 [US4] Run all four suites, then quickstart §4 SC-009 by hand against the real project. **Automated part done** (`:domain:test`, `:app:test`, `:data:connectedAndroidTest` 106/106, `:app:connectedAndroidTest` 74/74, all green). The real-project manual check is a human task, same as T072/T089/T109 — left unchecked.
...
- [ ] T131 [P] Write `data/src/androidTest/kotlin/com/giraffe/mizanapp/data/SupabaseRemoteDataSourceContractTest.kt`: a thin live test, skipped with `Assume.assumeTrue` when `BuildConfig.SUPABASE_URL` is blank. Assert one upsert round-trips, a duplicate upsert leaves one row, a tombstone upsert cannot be cleared, and a day-record upsert with a higher version does not raise the stored version. **Blocked in this environment**: every write path requires an authenticated session (RLS ties rows to `auth.uid()`), which requires a real OTP round trip to an inbox this environment cannot read. Needs a human to run once, same boundary research R14 already draws.
