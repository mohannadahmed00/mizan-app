...
- [ ] T130 Re-run the row-level-security verification as the merge gate (SC-008): `supabase db execute --file specs\007-identity-cloud-sync\contracts\rls-verification.sql`. Must print `RLS OK`. Confirm `supabase/migrations/0001_identity_cloud_sync.sql` is still byte-identical to `specs/007-identity-cloud-sync/contracts/remote-schema.sql`. **Second half done 2026-08-29**: `diff supabase/migrations/0001_identity_cloud_sync.sql specs/007-identity-cloud-sync/contracts/remote-schema.sql` returns no output — byte-identical. **The `RLS OK` run is still blocked**: the project database is unreachable, timing out on a bare `select 1` through the Supabase MCP while the management API answers normally, which reads as a paused project. The MCP exposes no resume operation, and neither the `supabase` CLI nor `psql` is installed here. Needs the project resumed, then either the CLI run above or the script executed in one session through the MCP.
