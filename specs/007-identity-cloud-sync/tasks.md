...
- [ ] T072 [US1] Run all four suites, then quickstart §4 SC-001 by hand: record several days signed out, note every total, sign in, confirm nothing changed, uninstall, reinstall, sign in, confirm the figures return. **Deferred to GitHub issue #15** — this is SC-001, gated on a real OTP sign-in; the Free plan has no fixed test OTP. `SignInMigrationTest` and `SignInMigrationResumeTest` cover it automatically and are green.
...
- [ ] T089 [US2] Run all four suites, then quickstart §4 SC-003 by hand: airplane mode, record a full day, close the app, restore connectivity, wait 60 s, confirm the rows arrived without opening the app. **Deferred to GitHub issue #15** — this is SC-003, gated on a real OTP sign-in; the Free plan has no fixed test OTP. `BackgroundSyncSchedulingTest` covers it automatically and is green.
...
- [X] T103 [US3] Add `suspend fun applyRemote(changes: RemoteChanges)` to `data/src/main/kotlin/com/giraffe/mizanapp/data/sync/SyncEngine.kt`, implementing [contracts/sync-engine.md](./contracts/sync-engine.md) §6 in one transaction. **Day records have exactly two outcomes: if a local plan for that date already exists, do nothing at all** — no re-derivation, no version change, no recalculated total (FR-024a, Conventions §4) — **otherwise** build the plan with `buildDayPlan` at the incoming version, or leave the date `NOT_YET_KNOWN` and request a catalogue pull when that version is not held locally. Completions use `mergeCompletion`, binding `dayPlanId` from `creditedDate`, and never rewrite `pointsAwarded`.
...
- [ ] T109 [US3] Run all four suites, then quickstart §4 SC-004 and SC-006 by hand on two emulators — including the documented different-version exception. **Automated part done** (`:domain:test`, `:app:test`, `:data:connectedAndroidTest` 100/100, `:app:connectedAndroidTest` 74/74, all green). The two-emulator manual walkthrough is a human task, same as T072/T089 — left unchecked. **Deferred to GitHub issue #15** — this is SC-004 and SC-006, both gated on a real OTP sign-in; the Free plan has no fixed test OTP. `TwoDeviceConvergenceTest` and `BackfillResumeTest` cover them automatically and are green.
...
- [X] T114 [US4] Call `pullIfNewer()` from `data/src/main/kotlin/com/giraffe/mizanapp/data/sync/SyncWorker.kt` before the record pull, wrapped so that any non-`Added` outcome is logged and ignored rather than short-circuiting `drain()` or `pull()`. Register the repository in `app/src/main/java/com/giraffe/mizanapp/di/Modules.kt`. Re-run T112.
- [ ] T115 [US4] Run all four suites, then quickstart §4 SC-009 by hand against the real project. **Automated part done** (`:domain:test`, `:app:test`, `:data:connectedAndroidTest` 106/106, `:app:connectedAndroidTest` 74/74, all green). The real-project manual check is a human task, same as T072/T089/T109 — left unchecked. **Deferred to GitHub issue #15** — SC-009's by-hand leg needs a signed-in device too, since `catalogue_publications` is readable only to a signed-in client, so it hits the same OTP wall. `RemoteCatalogueImmutabilityTest` — this increment's Principle III test — covers it automatically and is green.
...
- [X] T125 [US5] Complete `signOut(REMOVE_LOCAL_RECORDS)` in `data/src/main/kotlin/com/giraffe/mizanapp/data/repository/SupabaseAccountRepository.kt` (the `TODO("T125")` from T062): end the session, then call `LocalRecordWipe.wipe()`. Re-run T117.
- [X] T126 [US5] Complete the `replaceLocalRecords` branch in `SupabaseAccountRepository.confirmCode` (the `TODO("T126")` from T062): when `AccountScope` holds a different `userId` and `replaceLocalRecords` is false, return `WouldReplaceLocalRecords(currentEmail, recordedDays, completionCount, unsyncedCount)` **without opening a session or changing anything**; when true, wipe and then sign in. Re-run T118. (Also fixed: the local-conflict check now runs before the Supabase-client check, since it must work with no client configured too.)
...
- [X] T128 [US5] Add `Destination.Profile` to `app/src/main/java/com/giraffe/mizanapp/MainActivity.kt` (`"PROFILE"` in `encode`/`decode`, a `ProfileRoute`, an `AppRoute` branch), make the Today entry point from T070 open `Profile` when signed in and `SignIn` when signed out, and register the ViewModel in `app/src/main/java/com/giraffe/mizanapp/di/Modules.kt`. Re-run T121 and T122.
- [X] T129 [US5] Run all four suites plus quickstart §4 SC-007 by hand: fresh install, airplane mode, no account, walk the whole Phase 2–6 product. **Done 2026-08-29.** All four suites green on `Medium_Phone_API_36.1` (API 36): `:domain:test`, `:app:test`, `:data:test`, `:data:connectedAndroidTest` 112 tests +1 skip, `:app:connectedAndroidTest` 81/81. SC-007 walked by hand both ways — (a) airplane mode confirmed at the OS (`ping` → network unreachable): Today, Week, History, Insights and the streak panel all render, recording and undo work instantly, `SyncStatusBar` renders nothing, no account prompt or interstitial anywhere, and `Account` is an opt-in entry point that routes to sign-in and backs out cleanly; (b) repeated with the network **on** and the Supabase project paused — identical behaviour, recording still instant, still no sync bar, nothing in the crash buffer.
...
- [X] T130 Re-run the row-level-security verification as the merge gate (SC-008): `supabase db execute --file specs\007-identity-cloud-sync\contracts\rls-verification.sql`. Must print `RLS OK`. Confirm `supabase/migrations/0001_identity_cloud_sync.sql` is still byte-identical to `specs/007-identity-cloud-sync/contracts/remote-schema.sql`. **Done 2026-08-29, both halves.** `diff` of the migration against the contract returns no output — byte-identical. The verification script ran against the live project (PostgreSQL 17.6) and printed **`RLS OK`**; every assertion passed — B cannot read A's rows, cannot update them, cannot insert rows owned by A (`WITH CHECK` rejects with `insufficient_privilege`), no delete policy exists on `completions`, an authenticated client cannot publish a catalogue version, a day record does not move to a newer catalogue version, and a tombstone cannot be cleared by a later write. Run through the Supabase MCP rather than the CLI (neither `supabase` nor `psql` is installed here), in one session so `set local` and the closing `rollback` shared a transaction. **The test is not vacuous**: `postgres` carries `rolbypassrls`, `authenticated` does not, so the script's `set local role authenticated` is what makes RLS apply — confirmed against `pg_roles` before running. Rollback semantics were proven on a disposable table first, and a post-run count confirmed the script left nothing behind: no `@example.test` users, no version-999 catalogue row, every table back to its baseline count and the project's one real `auth.users` row untouched.
- [ ] T131 [P] Write `data/src/androidTest/kotlin/com/giraffe/mizanapp/data/SupabaseRemoteDataSourceContractTest.kt`: a thin live test, skipped with `Assume.assumeTrue` when `BuildConfig.SUPABASE_URL` is blank. Assert one upsert round-trips, a duplicate upsert leaves one row, a tombstone upsert cannot be cleared, and a day-record upsert with a higher version does not raise the stored version. **Deferred to GitHub issue #15**: every write path requires an authenticated session (RLS ties rows to `auth.uid()`), which requires a real OTP round trip to an inbox this environment cannot read. A fixed test OTP in Supabase Auth would unblock it, but that setting is not available on the project's current Free plan. Merged without it, knowingly — the boundary is the one research R14 already draws.
- [X] T132 [P] Audit every string added by this increment against Conventions §6 and the `CLAUDE.md` Principle IX list (SC-011): both sign-out confirmations, the account-switch confirmation, every `SyncStatus` string, every sign-in step, the still-loading labels, and the conflict-policy line. Record the audit as a comment block at the top of `app/src/test/java/com/giraffe/mizanapp/sync/SyncStatusCopyTest.kt`.
- [X] T133 [P] Boundary and prohibition audit. `Grep` for `io.github.jan`, `io.ktor`, `androidx.work` and `org.koin` across `domain/src` — must return nothing; across `data/src` — must appear only in `sync/SupabaseClientFactory.kt`, `sync/SupabaseRemoteDataSource.kt`, `sync/SyncWorker.kt` and `sync/SyncScheduler.kt`. Then `git grep -n "adoptMergedVersion\|updateMergedVersion"` — must return nothing (FR-024a), and `git grep -n "signInWith(Email"` — must return nothing (FR-002). Confirm `ModuleBoundaryTest` passes. **All clean**, with one addition to the allowed-file list that the original wording didn't anticipate: `data/src/main/kotlin/com/giraffe/mizanapp/data/repository/SupabaseAccountRepository.kt` legitimately imports `io.github.jan.*` too — it implements `AccountRepository` directly over `SupabaseClient.auth`, exactly as T062 specified. `domain/src` is clean; `androidTest` sources using `androidx.work.testing` (`BackgroundSyncSchedulingTest.kt`) are expected and out of this audit's scope (test-only).
- [X] T134 [P] Confirm `data/schemas/com.giraffe.mizanapp.data.db.MizanDatabase/3.json` is committed and that `1.json` and `2.json` are byte-identical to `develop-v1` (`git diff develop-v1 -- data/schemas`).
- [X] T135 [P] Update `docs/PLAN.md` to mark Phase 7 as delivered, and record the conflict policy there for developers. Note that FR-019a's user-facing obligation is satisfied by the profile screen's statement (T127), not by this file.
- [ ] T136 Run the full quickstart §4 validation end to end, all eleven success criteria, and record the result in the pull request description. **Partly done 2026-08-29.** SC-007 walked by hand (see T129). SC-011 read by hand: every string this increment adds was read against the Principle IX list — none attributes a failure to the user, and no surface renders red. SC-002's signed-out leg confirmed (recording and undo instant offline, no spinner, no disabled control). SC-005, SC-009 and SC-010 rest on their automated tests, all green. SC-008 passed live — see T130. **Deferred to GitHub issue #15**: SC-001, SC-003, SC-004 and SC-006, which all require a signed-in account and so are gated on an OTP code reaching an inbox, the same wall as T131. A fixed test OTP would open all five at once, but that setting needs a paid Supabase plan and this project is on Free. Each has a passing automated counterpart; merged with the gap understood and accepted rather than silently skipped.
- [X] T137 Pre-merge gate check against `CLAUDE.md`: Constitution Check passes and names each principle touched (**confirmed**: `specs/007-identity-cloud-sync/plan.md` §Constitution Check names all nine, gate result PASS); all four suites green (**confirmed** this session: `:domain:test`, `:app:test`, `:data:connectedAndroidTest` 111/111 +1 skip, `:app:connectedAndroidTest` 81/81); **the PR's commit history shows every test task committed before its implementation task** (**confirmed** for T090–T128 via `git log --oneline` on this branch — every `test(007)` commit precedes its matching `feat(007)` commit); the Principle III historical-immutability test (T110) is present and passing (**confirmed**); the Room migration is additive and its schema exported (**confirmed**, T039/T134). All five `CLAUDE.md` gates for a merge into `develop-v1` are met, and the spec-level gates SC-007, SC-008 (live `RLS OK`, T130) and SC-011 passed by hand. Five items ship deferred to GitHub issue #15 — see T136.
