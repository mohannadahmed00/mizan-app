...
17 rules, 17 defect variants, 16 fixture files. Rule 17 (`NoCatalogue`) has no fixture — its
fixture is the absence of one. There is also one extra fixture, `two-defects.json`, which is **not**
a rule fixture; it proves multi-defect reporting only.
...
- [ ] T006 [TEST] [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/CatalogueJsonTest.kt`. Write one test `good fixture parses` that loads `/catalogue/good/valid-catalogue.json` from the classpath via `javaClass.getResourceAsStream`, calls `parseCatalogue(json)`, and asserts the result is a success containing 1 version, 10 sections, 40 tasks and 40 taskVersions. Run `./gradlew :app:testDebugUnitTest`. **Expected: COMPILE FAILURE** — neither `parseCatalogue` nor `Catalogue` exists, and the fixture file does not exist. **This is the red state. Observe it before continuing.**

### 3b — The good fixture

- [ ] T007 [US1] Create `app/src/test/resources/catalogue/good/valid-catalogue.json` with **exactly** the content below. Do not alter a single number. The arithmetic is load-bearing: 40 tasks, 10 sections, 69/74/76 per day, 500 per week.
...
- [ ] T009 [P] [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/model/Section.kt`. `@Serializable data class Section(val id: String, val label: String, val order: Int)`.

- [ ] T010 [P] [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/model/TaskDefinition.kt`. `@Serializable data class TaskDefinition(val slug: String, val sectionId: String, val displayPosition: Int, val label: String)`.

- [ ] T011 [P] [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/model/CatalogueVersion.kt`. `@Serializable data class CatalogueVersion(val version: Int, @Serializable(with = LocalDateSerializer::class) val effectiveFrom: LocalDate)`. In the same file add `object LocalDateSerializer : KSerializer<LocalDate>` serialising as an ISO-8601 `String` via `LocalDate.parse` and `toString`.

- [ ] T012 [P] [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/model/TaskVersion.kt`. `@Serializable data class TaskVersion(val taskSlug: String, val catalogueVersion: Int, val points: Int, val maxOccurrencesPerDay: Int, val scheduleRule: ScheduleRule)`.

- [ ] T013 [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/model/Catalogue.kt`. `@Serializable data class Catalogue(val versions: List<CatalogueVersion>, val sections: List<Section>, val tasks: List<TaskDefinition>, val taskVersions: List<TaskVersion>)`.

### 3d — Parser (makes T006 green)

- [ ] T014 [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/CatalogueJson.kt` with `fun parseCatalogue(json: String): Result<Catalogue>`. Use `Json { ignoreUnknownKeys = false; classDiscriminator = "type" }` and wrap parsing in `runCatching`. Run `./gradlew :app:testDebugUnitTest`. **Expected: T006 now PASSES.**

### 3e — Validator skeleton

- [ ] T015 [TEST] [US1] Add a test `valid catalogue has no defects` in a new `app/src/test/java/com/giraffe/mizanapp/catalogue/CatalogueValidatorTest.kt`: parse the good fixture, call `CatalogueValidator().validate(catalogue)`, assert the returned list is empty. Run tests. **Expected: COMPILE FAILURE** — `CatalogueValidator` and `CatalogueDefect` do not exist. Observe the red.

- [ ] T016 [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/CatalogueDefect.kt`. Declare `sealed interface CatalogueDefect` with exactly these 17 data classes, each carrying the listed fields:
...
- [ ] T017 [US1] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/CatalogueValidator.kt` with `class CatalogueValidator { fun validate(catalogue: Catalogue): List<CatalogueDefect> = emptyList() }`. Run tests. **Expected: T015 PASSES.** The validator is deliberately empty — rules are added one at a time below.
...
- [ ] T018 [US1] **Rule 1 — duplicate slug.** Fixture `bad/duplicate-slug.json`: change the `slug` of task `fajr-2` to `fajr-1` (and its taskVersion `taskSlug` likewise). Expect `DuplicateTaskSlug("fajr-1", 2)`.

- [ ] T019 [US1] **Rule 2 — malformed slug.** Fixture `bad/malformed-slug.json`: change task `asr-1`'s slug to `Asr_1` (and its taskVersion). Expect `MalformedSlug("Asr_1")`. Valid pattern: `^[a-z0-9]+(-[a-z0-9]+)*$`.

- [ ] T020 [US1] **Rule 3a — zero points.** Fixture `bad/zero-points.json`: set `points` to `0` for `fajr-1`. Expect `NonPositivePoints("fajr-1", 0)`.

- [ ] T021 [US1] **Rule 3b — negative points.** Fixture `bad/negative-points.json`: set `points` to `-2` for `fajr-1`. Expect `NonPositivePoints("fajr-1", -2)`.

- [ ] T022 [US1] **Rule 4 — occurrence limit.** Fixture `bad/zero-occurrences.json`: set `maxOccurrencesPerDay` to `0` for `witr`. Expect `InvalidOccurrenceLimit("witr", 0)`.

- [ ] T023 [US1] **Rule 5 — unknown section.** Fixture `bad/missing-section.json`: set `sectionId` of `isha-1` to `nonexistent`. Expect `UnknownSection("isha-1", "nonexistent")`.

- [ ] T024 [US1] **Rule 6 — duplicate position in section.** Fixture `bad/duplicate-position-in-section.json`: set `displayPosition` of `adhkar-2` to `1` (colliding with `adhkar-1`). Expect `DuplicateDisplayPosition("adhkar", 1, listOf("adhkar-1", "adhkar-2"))`. **Also add the negative control**: assert the good fixture stays clean, proving that two tasks in *different* sections sharing position 1 is not a defect (the good fixture contains many).

- [ ] T025 [US1] **Rule 7 — duplicate section order.** Fixture `bad/duplicate-section-order.json`: set `order` of section `dhuhr` to `1`. Expect `DuplicateSectionOrder(1, listOf("fajr", "dhuhr"))`.

- [ ] T026 [US1] **Rule 8 — unreachable schedule.** Fixture `bad/unreachable-schedule.json`: set `friday-1`'s scheduleRule to `{ "type": "daysOfWeek", "days": [] }`. Expect `UnreachableSchedule("friday-1")`.

- [ ] T027 [US1] **Rule 9 — duplicate version number.** Fixture `bad/duplicate-version-number.json`: add a second version entry `{ "version": 1, "effectiveFrom": "2026-06-01" }` — same `version` integer as the existing one, different date. Expect `DuplicateVersionNumber(1, 2)`.

- [ ] T028 [US1] **Rule 10 — version/date order mismatch.** Fixture `bad/version-order-mismatch.json`: add `{ "version": 2, "effectiveFrom": "2025-01-01" }` — a higher version taking effect earlier. Expect `VersionOrderMismatch(2, "2025-01-01")`.

- [ ] T029 [US1] **Rule 11 — duplicate effective-from.** Fixture `bad/duplicate-effective-from.json`: add `{ "version": 2, "effectiveFrom": "2026-01-01" }` — same date as version 1. Expect `DuplicateEffectiveFrom("2026-01-01", listOf(1, 2))`.

- [ ] T030 [US1] **Rule 12 — weekday totals.** Fixture `bad/wrong-weekday-total.json`: change `qiyam` points from `5` to `6`. Expect `WeekdayTotalMismatch` for every weekday (base day becomes 70). Rule: for each of the 7 weekdays compute available points; expected are Mon 74, Tue 69, Wed 69, Thu 74, Fri 76, Sat 69, Sun 69.

- [ ] T031 [US1] **Rule 13 — week total.** Fixture `bad/wrong-week-total.json`: change `friday-1` points from `1` to `2`. Expect `WeekTotalMismatch(500, 501)`.

- [ ] T032 [US1] **Rule 14 — section composition.** Fixture `bad/wrong-section-composition.json`: delete task `adhkar-9` and its taskVersion. Expect `SectionCompositionMismatch("adhkar", 18, 16)`. Expected section totals on a base day: fajr 12, dhuhr 8, asr 6, maghrib 6, isha 6, qiyam-witr 9, quran 4, adhkar 18.

- [ ] T033 [US1] **Rule 15 — user authoring affordance.** Fixture `bad/user-editable-flag.json`: add `"editable": true` to task `fajr-1`. Add `fun scanForAuthoringAffordances(json: String): List<CatalogueDefect>` to `CatalogueJson.kt`. It scans the **raw JSON text** for these exact key names at any nesting level — `editable`, `userCreated`, `custom`, `deletable`, `sortable`, `reorderable`, `ownerId`, `userId` — and returns `UserAuthoringAffordance(field)` for each hit. **This scan runs before `parseCatalogue`**; if it returns anything, the strict parse is not consulted. Expect `UserAuthoringAffordance("editable")`, **not** `MalformedCatalogue`. Without this ordering the defect is unreachable, because `ignoreUnknownKeys = false` would turn the field into a generic parse error and FR-019 would lose its name.

- [ ] T034 [US1] **Rule 16 — malformed catalogue.** Fixture `bad/malformed.json`: content is the single line `{ "versions": [` (truncated, invalid JSON). Expect `MalformedCatalogue`. Confirm the authoring scan finds nothing here, so this falls through to the parse.

- [ ] T035 [US1] **Rule 17 — no catalogue.** No fixture file. Add a test calling the loader with a path that does not exist, asserting `NoCatalogue` is returned — **not** an empty list, and not an exception. This is FR-011.

### 3g — Cross-cutting guarantees

- [ ] T036 [US1] Add a test `validator never throws` in `CatalogueValidatorTest.kt` running `validate` against every fixture in `bad/`, asserting no exception escapes.
...
- [ ] T041 [US1] Run the full suite: `./gradlew :app:testDebugUnitTest`. **Expected**: all tests pass. Then verify the suite is real — edit `good/valid-catalogue.json` changing one `"points": 2` to `"points": 3`, re-run, confirm `CatalogueArithmeticTest` FAILS, then `git checkout -- app/src/test/resources/catalogue/good/valid-catalogue.json`.
...
- [ ] T048 [P] Create `app/src/test/java/com/giraffe/mizanapp/catalogue/DomainPurityTest.kt`. Read every `.kt` file under `app/src/test/java/com/giraffe/mizanapp/catalogue/` as text and assert none contains the string `import android.`. Guards Principle II and keeps the Phase 2 move into `:domain` mechanical.

- [ ] T049 Verify no production code leaked. Run `git diff develop-v1... --stat -- app/src/main`. **Expected: empty output.** Any output violates SC-007 — find and remove it.
...
- [ ] T051 Verify no Gradle module was created (FR-018). Confirm `settings.gradle.kts` still contains exactly one `include` line, `include(":app")`. No `:domain`, no `:data`.

- [ ] T052 Run the full suite one final time: `./gradlew :app:testDebugUnitTest`. **Expected**: BUILD SUCCESSFUL, every test green.

- [ ] T053 Walk [quickstart.md](./quickstart.md) end to end, including the deliberate-break step in Scenario 4. Confirm each stated expectation holds.
